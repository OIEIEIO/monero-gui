...
MONERO_DIR=monetaverde
MONEROD_EXEC=monetaverded
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/monetaverde-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=monetaverded.exe
fi
...
$QMAKE ../monetaverde-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 
