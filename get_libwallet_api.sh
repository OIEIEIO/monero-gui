#!/bin/bash
MONERO_URL=https://github.com/OIEIEIO/monetaverde.git
...
MONERO_DIR=$ROOT_DIR/monetaverde
BUILD_LIBWALLET=false

# init and update monero submodule
if [ ! -d $MONERO_DIR/src ]; then
    git submodule init monetaverde
fi
git submodule update --remote
git -C $MONERO_DIR fetch
git -C $MONERO_DIR checkout 2.0.0
...
git -C $MONERO_DIR config user.name "MonetaVerde GUI"
git -C $MONERO_DIR config user.email "gui@mcn.green"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging monetaverde push request #$PR"
    # fetch pull request and merge
    git -C $MONERO_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $MONERO_DIR merge --quiet PR-$PR  -m "Merge monetaverde PR #$PR"
...
# Build libwallet if no previous version file exists
    ...
    echo "monetaverde/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
